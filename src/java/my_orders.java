/*
 * To change this template, choose Tools | Templates
 * and open the template in the editor.
 */

import java.io.IOException;
import java.io.PrintWriter;
import java.sql.Connection;
import java.sql.Statement;
import javax.servlet.ServletException;
import javax.servlet.http.HttpServlet;
import javax.servlet.http.HttpServletRequest;
import javax.servlet.http.HttpServletResponse;
import javax.servlet.http.HttpSession;

/**
 *
 * @author PRACHI
 */
public class my_orders extends HttpServlet {

    
      Connection cn = null;
     Statement st = null;

    @Override
    protected void doPost(HttpServletRequest req, HttpServletResponse resp) throws ServletException, IOException
    { 
        PrintWriter out= resp.getWriter();
        HttpSession session = req.getSession();
        String id = req.getParameter("id"); 
        String watch_name=req.getParameter("watch_name");
        String item = req.getParameter("item");
        String product = req.getParameter("product");
        String quantity = req.getParameter("quantity");
        String price = req.getParameter("price");
        String delivery_date = req.getParameter("delivery_date");
        String event = req.getParameter("submit");
        
        
        out.println(item);
        out.println(product);
        out.println(quantity);
        out.println(price);
        out.println(delivery_date);
        out.println(event);
        
        
         Database db = new Database();
        String result = db.Connectdb();
        out.println(result);
        
        
        /*if(event.equals(""))
        {
            if(item.equals("")||product.equals("")||quantity.equals("")||price.equals("")|| cancle_order.equals("")|| delivery_date.equals(""))
            {
                 resp.setContentType("text/html");
                   out.println("   <script type=\"text/javascript\">");
                   out.println("   alert('Some Fields Are Empty');     ");
                   out.println("   location='my_orders.jsp';  ");
                   out.println("   </script>               ");
            }
            else
            {
                try
                {
                    String sql = "insert into my_orders(item, product, qauntity, price, cancle_order, delivery_date,total, user_id)values('"+item+"','"+product+"','"+quantity+"','"+price+"','"+cancle_order+"','"+delivery_date+"' ,'"+session.getAttribute("user_id") +"')";
                    String Insert = db.Insert(sql);
                    out.println(Insert);
                    resp.setContentType("text/html");
                   out.println("   <script type=\"text/javascript\">");
                   out.println("   alert('Record inserted successfully');     ");
                   out.println("   location='home_page.jsp';  ");
                   out.println("   </script>               ");
                }
                catch(Exception ex)
                {
                    out.println(ex.toString());
                }
            }
        }

     */   
         
         
         if(event.equals("Cancel"))
        {
            if(id.equals(""))
            {
                   resp.setContentType("text/html");
                   out.println("<script type=\"text/javascript\">");
                   out.println("alert('Some Fields Are Empty');     ");
                   out.println("location='my_orders.jsp';  ");
                   out.println("</script>               ");
            }
            else
            {
                try
                {     
                    String sql = "delete from my_orders where id = '"+id+"' ";
                    String delete = db.delete(sql);
                    out.println(delete);
                    
                  resp.setContentType("text/html");
                  out.println("<script type=\"text/javascript\">");
                  out.println("alert('Product Deleted successfully');     ");
                  out.println("location='my_orders.jsp';  ");
                  out.println("</script>               ");
                }
                catch(Exception ex)
                {
                    out.println(ex.toString());
                }
            }
        }
        
        if(event.equals("Bookings"))
        {
            session.setAttribute("id",id);
            resp.sendRedirect("bookings_admin2.jsp");
        }
        
    }
    

}
         
         
         
         
         
        
       
    
    


    

    
